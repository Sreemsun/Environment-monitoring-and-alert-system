...
# Get the parent directory (project root) and set static folder path
...
# Configuration - Load from environment variables with fallback
WEATHER_API_KEY = os.getenv('WEATHER_API_KEY', '3ab9567742ab4f0d91192045263101')
WEATHER_API_BASE = 'https://api.weatherapi.com/v1'
OPENAQ_API_BASE = 'https://api.openaq.org/v2'

# OpenAI API Configuration - Load from environment variables
OPENAI_API_KEY = os.getenv('OPENAI_API_KEY', '')
USE_AI_API = bool(OPENAI_API_KEY)  # AI-powered responses enabled if key is present
...
def get_historical(location):
    ...
    try:
        ...
        hours = min(hours, 720)  # Max 30 days for calendar
        
        # First, try to get data from database
        historical = get_historical_data(location, hours)
        
        # For calendar requests (96+ hours), if insufficient data, fetch from Weather API
        # For smaller requests, require at least 30% of requested hours
        if hours >= 96:
            required_minimum = max(10, hours * 0.3)  # At least 30% of requested hours
        else:
            required_minimum = max(5, hours * 0.5)  # At least 50% for smaller requests
            
        if not historical or len(historical) < required_minimum:
            print(f"📥 Insufficient data in database ({len(historical) if historical else 0}/{hours} hours). Fetching from API for {location}")
            historical = fetch_historical_from_api(location, hours)
        
        if not historical:
            return jsonify({
                'success': False,
                'message': f'No historical data available for {location}',
                'data': []
            })
...
def fetch_historical_from_api(location, hours):
    """Fetch historical data from Weather API for any location"""
    try:
        days = min(7, (hours // 24) + 1)  # Weather API supports max 7 days history
        historical_data = []
        ...
        for day_offset in range(days):
            ...
            params = {
                ...
                'q': location,
                'dt': target_date,
                'aqi': 'yes'  # Important: Include air quality data
            }
            
            print(f"  Fetching {target_date} for {location}...")
            response = requests.get(url, params=params, timeout=10)
            ...
            if response.status_code == 200:
                ...
                print(f"  ⚠️ Failed to fetch {target_date}: Status {response.status_code}")
        ...
        if len(historical_data) > hours:
            historical_data = historical_data[-hours:]
        
        # Log air quality stats
        pm25_values = [d['pm2_5'] for d in historical_data if d['pm2_5'] > 0]
        print(f"✅ Fetched {len(historical_data)} readings from API for {location}")
        print(f"   PM2.5 values: {len(pm25_values)} non-zero readings")
        if pm25_values:
            print(f"   PM2.5 range: {min(pm25_values):.1f} - {max(pm25_values):.1f}")
...
@app.route('/api/locations/ranking', methods=['GET'])
def get_locations_ranking():
    """
    Calculate and return average AQI for all locations in database
    Sorted from best (lowest AQI) to worst (highest AQI) air quality
    """
    try:
        ...
        # Query to get average PM2.5 for each location from last 24 hours
        query = '''
            SELECT 
                location_name,
        ...
        # Convert PM2.5 to AQI using EPA standard
        ...
        def get_aqi_category(aqi):
            ...
            if aqi <= 50:
                return {'category': 'Good', 'color': '#4caf50', 'icon': '😊'}
            elif aqi <= 100:
                return {'category': 'Moderate', 'color': '#ffeb3b', 'icon': '😐'}
            elif aqi <= 150:
                return {'category': 'Unhealthy for Sensitive Groups', 'color': '#ff9800', 'icon': '😷'}
            elif aqi <= 200:
                return {'category': 'Unhealthy', 'color': '#f44336', 'icon': '😨'}
            elif aqi <= 300:
                return {'category': 'Very Unhealthy', 'color': '#9c27b0', 'icon': '☠️'}
            else:
                return {'category': 'Hazardous', 'color': '#880e4f', 'icon': '💀'}
        
        # Process results and fetch country information
        rankings = []
        for row in results:
            ...
            # Fetch country information from Weather API for this location
            country = 'Unknown'
            try:
                url = f"{WEATHER_API_BASE}/current.json"
                params = {
                ...
                print(f"  ⚠️ Could not fetch country for {row['location_name']}: {e}")
            ...
            rankings.append({
                ...
                'avg_humidity': round(row['avg_humidity'], 0) if row['avg_humidity'] else None,
        ...
        print(f"✅ Generated ranking for {len(rankings)} locations")
        ...
        print(f"❌ Error in get_locations_ranking: {e}")
        return jsonify({'success': False, 'error': str(e)}), 500
...
def get_nearby_stations():
    """
    Get nearby air quality stations within a radius of a given location
    Returns up to 5 stations with their AQI values
    """
    import math
    ...
    radius_km = request.args.get('radius', default=80, type=int)
    limit = request.args.get('limit', default=5, type=int)
    
    if not location and (not lat or not lon):
        return jsonify({
            'success': False,
            'error': 'Either location name or lat/lon coordinates required'
        }), 400
    ...
    try:
        ...
        print(f"🔍 Searching for stations near {location_name} (lat={lat}, lon={lon}, radius={radius_km}km)")
        
        # Define a grid of sample locations around the search point
        # In a real implementation, you would query a database or external API
        # For now, we'll generate some nearby points using the monitored locations
        # and calculate virtual stations around the search area
        ...
        # Generate nearby virtual stations (in real app, query actual air quality stations)
        nearby_stations = []
        
        # Create stations spread across 80km radius with varying distances
        # 1 degree latitude ≈ 111 km, so we use larger offsets to get different locations
        import random
        random.seed(int(lat * 1000 + lon * 1000))  # Consistent results for same location
        ...
        offsets = [
            ...
            (-0.08, -0.62, 'Far West'),
            # Diagonal far points
            (0.48, 0.45, 'Far Northeast'),
            (-0.52, 0.50, 'Far Southeast'),
        ]
        
        for lat_offset, lon_offset, direction in offsets:
            station_lat = lat + lat_offset
            station_lon = lon + lon_offset
                ...
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        nearby_stations.append({
                            ...
                            'aqi': aqi,
                            'pm2_5': round(pm25, 2),
                            'pm10': round(aqi_data.get('pm10', 0), 2),
                            'no2': round(aqi_data.get('no2', 0), 2),
                            'o3': round(aqi_data.get('o3', 0), 2),
                            'temperature': current.get('temp_c'),
                            'humidity': current.get('humidity')
                        })
                        
                        print(f"  ✅ Station: {station_name}, AQI: {aqi}, Distance: {distance:.2f}km")
                        
                except Exception as station_error:
                    print(f"  ⚠️ Error fetching station data: {station_error}")
                    continue
        ...
        print(f"✅ Found {len(nearby_stations)} stations within {radius_km}km")
